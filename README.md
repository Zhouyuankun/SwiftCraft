...
SwiftCraft 是一个使用 Swift、AppKit、MetalKit 和 Metal Shading Language 编写的 macOS 体素渲染 Demo。当前程序会根据随机种子生成连续起伏的体素地形，玩家可以使用第一人称自由相机在其中移动；世界会围绕玩家动态创建、加载和卸载区块网格。

本文档以当前代码为准，用于帮助后续开发者快速理解项目。它描述的是已经实现的行为，不是产品规划。

## 当前运行效果

程序启动后显示一个由草方块、泥土和石头组成的程序化体素世界，包含平原、低地和丘陵。初始相机位于区块 `(0, 0)` 的地表上方。点击窗口后进入操作模式，可以在世界中自由飞行；右上角 HUD 会显示相机位置、朝向、当前区块坐标和本次运行的随机世界种子。
...
- `16 × 64 × 16` 的 Chunk 方块数据结构，使用一维连续数组存储
...
- 显示位置、朝向、Chunk 坐标、准心目标方块类型和世界种子的调试 HUD
- 屏幕中心十字准心，以及准心射线命中方块时的偏白高亮
- 左键破坏准心命中的方块，并持久保留本次运行期间的修改
- 加载纹理时自动生成 mipmap（当前采样器尚未启用 mip 级过滤）
- Chunk 集合变化时记录地形数据生成和网格构建耗时
- 固定面向明暗：顶面、两组侧面和底面使用不同亮度以强化方块轮廓
...
暂停菜单使用不带填充色的透明高斯模糊层覆盖游戏内容区，中央面板使用 macOS 26 原生液态玻璃效果，两个操作按钮使用完全不透明的系统强调色并提供悬停反馈。再次按 Escape 或点击“继续游戏”都会关闭菜单、重新锁定光标并恢复游戏焦点；“退出游戏”会终止整个应用。
...
方块数据按 `(y * depth + z) * width + x` 存储在一维连续数组中，通过 Chunk 下标访问。`Chunk` 本身只创建空气数据容器，当前新区块由 `HeightmapTerrainGenerator` 使用绝对世界坐标生成：

```text
y = surfaceHeight       草方块（1 层）
y = surfaceHeight-3...  泥土（3 层）
y = 1...                 石头
y = 0                    基岩（1 层）
```

地表高度由四个使用独立盐值的二维 value noise/fBm 通道组合：低频大陆轮廓形成低地和宽缓地势，细节层打破规则平台，区域遮罩决定丘陵分布，脊状层塑造明显山脊。丘陵通过 `smoothstep` 与平原连续混合，最高地表限制为 `y = 56`。采样使用绝对世界坐标，因此跨 Chunk 连续并且不受加载顺序影响。世界种子在每次启动时随机生成，在单次运行内保持不变，并显示在 HUD、输出到控制台。

当前将 `y = 25` 作为材质分界海平面：地表不高于该高度的低地柱，顶部最多 4 层使用沙子；更高地表仍使用草和泥土。这个海平面不生成水，项目仍没有液体或透明方块渲染。

石层中的 `y = 1...24` 使用独立盐值的三维 value noise/fBm 生成煤矿。煤矿只替换原本的石头，并通过连续三维采样形成小型矿团；不会出现在基岩、泥土、草、沙或空气中。
...
`World.renderDistance` 当前为 1，因此 GPU 网格集合是以相机所在区块为中心的 `3 × 3` 区域。World 还会为这 9 个 Chunk 的四侧邻居预生成一圈仅含方块数据的 halo：初始通常准备 21 个数据 Chunk，但仍只构建和绘制 9 个网格。只有可见集合改变时才会执行加载和卸载。

需要注意：当前只是 GPU 网格流式加载。持续探索时，`loadedChunks` 通常维持 9 个，但 `chunkData` 会一直增长，并不具备无限世界所需的内存淘汰或磁盘持久化机制。

方块数据和 CPU 顶点数组通过两组受限并发后台队列生成；后台任务只使用值类型快照，完成结果由渲染线程统一接收。`chunkData`、`loadedChunks` 和 `MTLBuffer` 不会在后台线程直接修改。完成的网格进入待上传队列，按与玩家的距离排序，每帧最多创建 1 个 Chunk 的 `MTLBuffer`，预算为 2 ms。控制台用 `Chunk jobs` 记录调度数量，并用 `Async ready` 汇总后台耗时、待上传数量、实际应用及丢弃的网格结果。

每个网格任务携带 Chunk 版本号。玩家挖掘后版本递增，旧任务即使稍后完成也不会覆盖修改；快速离开可见范围时，完成的旧网格同样会被丢弃。已有网格会保留到对应的新版本完成，避免编辑时短暂闪空。

渲染器还会每 120 帧输出一次 `Frame CPU` 窗口统计，包括平均 CPU 帧耗时、最大值、超过 16.7 ms 和超过 33 ms 的帧数。该指标统计 `draw(in:)` 的 CPU 工作，不代表 GPU 完成时间。
...
- 邻居是实心方块：不生成该面。
- 邻居是空气：生成该面。
- 邻居越过 Chunk 边界：只查询当前可见集合中的相邻 Chunk。
- 相邻 Chunk 仅存在于数据 halo、尚不可见或垂直坐标超出世界高度：按空气处理，保留渲染外圈封边。

每个可见面由两个三角形组成，共生成 6 个非索引顶点。所有顶点在 CPU 上直接转换成世界坐标，然后合并进该 Chunk 的一个 `MTLBuffer`；渲染时不再使用单独的模型矩阵。

跨界时只为真正新进入可见范围的 Chunk 创建网格；已有 Chunk 不因 halo 或新可见邻居出现而重建。已有边界面进入两个实体 Chunk 内部后会被实体遮挡，不影响画面。玩家编辑水平边界方块时，当前 Chunk 和对应可见邻居仍会一起重建，保证新暴露面正确。
...
主纹理资源是 `Assets.xcassets/terrain_atlas.imageset/Blocks.png`，图片尺寸为 64 × 64，包含 4 × 4 个 16 × 16 图块。应用启动时会把图集拆成 16 个独立的纹理数组切片，顶点以切片编号选择材质，并使用缩进半个纹素的局部 UV，避免采样落在图块边界。

每个纹理数组切片单独生成 mipmap，因此低 mip 层不会混入相邻材质。地形片元着色器使用最近邻放大、缩小和 mip 级过滤，在保持像素风格的同时减少远距离闪烁。地形使用固定的面向亮度强化体素轮廓：顶面最亮、X/Z 侧面略暗且彼此有差异、底面最暗；这不是动态光照，目前仍没有法线、阴影、透明方块或颜色雾化。
...
- 初始 X/Z：`(8.0, 8.0)`；初始 Y 自动设为出生点地表高度加 3
- 初始 yaw：`-90°`，即面向 `-Z`
- 初始 pitch：`0°`
- 鼠标灵敏度：`0.15`
- 每帧移动步长：`0.15`
...
`GameViewController` 在运行时创建五个 `NSTextField`，固定在视图右上角，显示：

- `Position`：相机的 X、Y、Z，保留一位小数。
- `Facing`：水平视线在坐标轴上的符号组合，例如 `+X-Z`。
- `Chunk`：相机当前所在区块的 X、Z。
- `Block`：准心命中的 `BlockType` 枚举名称；未命中时显示 `air`。
- `Seed`：本次运行的无符号 64 位世界种子；重启应用会重新生成。

文本内容没有变化时不会重复赋值。

视图中央还有一个由两条白色矩形组成的固定十字准心。渲染器每帧从相机位置沿视线方向执行 3D DDA 体素射线检测，选择 6 格内的第一个非空气方块。命中后，地形片元着色器会将该方块颜色向白色混合 38%；未命中时不启用高亮。选取逻辑只依赖世界方块查询，不依赖具体地形生成规则。
...
├── Terrain.swift              地形配置、高度图生成器、二维噪声和统一坐标换算
...
- CPU 地形和顶点生成已移到后台；GPU Buffer 创建和结果合并仍发生在渲染线程，但已限制为每帧最多上传 1 个 Chunk。
- 移动速度依赖帧率；应引入 delta time 后再实现稳定的角色控制。
- `World.update` 每帧接收后台结果，并按距离优先从待上传队列处理最多 1 个网格。
